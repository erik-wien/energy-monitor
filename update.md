...
This app currently reads config from `config.ini`.
...
- [ ] Remove old config file (`config.ini`) and its example from the repo
...
- `config.yaml` is generated by `mcp/generate.py --app energie --target <target>`
