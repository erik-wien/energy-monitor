 ...
 * inc/config.php
 *
 * Energie config loader. Prefers config.yaml from the app root (generated by
 * mcp/generate.py, single source of truth across all environments) and falls
 * back to the legacy /opt/homebrew/etc/energie-config*.ini only when no
 * config.yaml is present. Both sources are normalised to the mcp schema:
 * db.{host,name,user,password}, auth_db.{...}, smtp.{...}, app.{base_url,env}.
 *
...
function energie_load_config(): array {
    ...
    // 1. config.yaml (generated by mcp/generate.py) sits one level above inc/.
    ...
    // 2. Legacy fallback for hosts still on the old /opt/homebrew/etc INIs.
    ...
    throw new RuntimeException(
        'Energie config not found: neither ' . $yamlPath
        . ' nor /opt/homebrew/etc/energie-config.ini'
    );
}
